...
function ecwid_square_get_sku_for_variation($square_token, $variation_id) {
    ...
    return $obj['item_variation_data']['sku'] ?? null;
}
...
function ecwid_square_ensure_variation_location_and_tracking($square_token, $variation_id, $location_id, $debug = false) {
    ...
    if ($var_needs_loc || $var_needs_tracking) {
        ...
        ec2sq_log($ok ? 'info' : 'error', 'ecwid->square', 'square_variation_presence_set', ['variation_id' => $variation_id, 'location' => $location_id, 'set_tracking' => $var_needs_tracking, 'set_location' => $var_needs_loc], null, null, $variation_id, $location_id);
        return $ok;
    }
...
|--------------------------------------------------------------------------
*/
function ec2sq_logs_get_all() {
    $logs = get_option('ec2sq_logs', []);
    if (!is_array($logs)) $logs = [];
    return $logs;
}
function ec2sq_logs_get_recent($limit = 200) {
    $all = ec2sq_logs_get_all();
    return array_slice(array_reverse($all), 0, max(1, intval($limit)));
}
function ec2sq_logs_clear() {
    update_option('ec2sq_logs', []);
}
function ec2sq_log($level, $direction, $event, $data = [], $rid = null, $sku = null, $variation_id = null, $location_id = null) {
    if (!get_option('ec2sq_logs_enabled', 1)) return;
    $entry = [
        'ts' => gmdate('Y-m-d\TH:i:s\Z'),
        'level' => strtolower($level),
        'dir' => $direction,
        'event' => $event,
        'rid' => $rid ?: ('log_' . bin2hex(random_bytes(4))),
        'sku' => $sku,
        'variation_id' => $variation_id,
        'location_id' => $location_id,
        'data' => is_array($data) ? $data : ['message' => strval($data)],
        'ip' => $_SERVER['REMOTE_ADDR'] ?? ''
    ];
    $logs = ec2sq_logs_get_all();
    $logs[] = $entry;
    $limit = (int)get_option('ec2sq_logs_limit', 1000);
    if (count($logs) > $limit) $logs = array_slice($logs, -1 * $limit);
    update_option('ec2sq_logs', $logs, false);
}
...
function ec2sq_run_sku_diagnostic($sku) {
    ...
    // Use cached Square items for performance
    $variation_id = null;
